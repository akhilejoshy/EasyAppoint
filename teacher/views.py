...
def cancel_appointment(request, id):
	if request.method == 'POST':
		...
		# Update the appointment with cancellation reason and mark it as canceled
		...
		return redirect('teacher_appointment_list')  # Redirect if cancellation request is not POST
...
def teacherhome(request):#this section for my appointment
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "Teacher" == group_name:
		user_name=request.user.get_username()#Getting Username
		...
		return render(request, 'teacher_home.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/') 
